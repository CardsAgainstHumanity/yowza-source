...
import { Badge, GroupBadge } from 'mastodon/components/badge';
import Button from 'mastodon/components/button';
import { FollowersCounter, FollowingCounter, ReferralsCounter, StatusesCounter } from 'mastodon/components/counters';
...
import ReferralCodeTout from '../../ui/components/referral_code_tout';

import { referralsEnabled } from 'mastodon/initial_state';
...
const messages = defineMessages({
  ...
  hideReblogs: { id: 'account.hide_reblogs', defaultMessage: 'Hide reyowzas from @{name}' },
  showReblogs: { id: 'account.show_reblogs', defaultMessage: 'Show reyowzas from @{name}' },
  enableNotifications: { id: 'account.enable_notifications', defaultMessage: 'Notify me when @{name} yowzas' },
  disableNotifications: { id: 'account.disable_notifications', defaultMessage: 'Stop notifying me when @{name} yowzas' },
...
const titleFromAccount = account => {
  ...
  const acct = account.get('acct') === account.get('username') ? `${account.get('username')}` : account.get('acct');
  ...
  render () {
    ...
    if (me !== account.get('id')) {
      if (signedIn && !account.get('relationship')) { // Wait until the relationship is loaded
        ...
        actionBtn = <Button disabled={account.getIn(['relationship', 'blocked_by'])} className={classNames({ 'button--destructive': account.getIn(['relationship', 'following']), 'unfollow':  account.getIn(['relationship', 'following']), 'follow': !account.getIn(['relationship', 'following']) })} text={intl.formatMessage(account.getIn(['relationship', 'following']) ? messages.unfollow : messages.follow)} onClick={signedIn ? this.props.onFollow : this.props.onInteractionModal} />;
      } else if (account.getIn(['relationship', 'blocking'])) {
    ...
    const acct            = isLocal && domain ? `${account.get('acct')}` : account.get('acct');
      ...
      <div className={classNames('account__header', { inactive: !!account.get('moved') })} onMouseEnter={this.handleMouseEnter} onMouseLeave={this.handleMouseLeave}>
        ...
        <div className='account__header__bar'>
          ...
          <div className='account__header__tabs__name'>
            <h1>
              <span className='account__header__tabs__name__badge'>
                <span dangerouslySetInnerHTML={displayNameHtml} />
                <VerifiedYowzaBadge badgeClass={account?.get('badge_class')} badgeCount={account?.get('total_checks')} />
              </span>
              <small>
          ...
          {!(suspended || hidden) && (
            <div className='account__header__extra'>
              <div className='account__header__bio' ref={this.setRef}>
                {(false) && <AccountNoteContainer account={account} />}

                {account.get('note').length > 0 && account.get('note') !== '<p></p>' && <div className='account__header__content translate' dangerouslySetInnerHTML={content} />}

                <div className='account__header__fields'>
                  {fields.filter((pair) => pair.get('value') != null && pair.get('value') != '').map((pair, i) => (
              ...
              <div className='account__header__extra__links'>
                ...
                <NavLink exact activeClassName='active' to='/leaderboard' title={intl.formatNumber(account.get('referrals_count'))}>
                  <ShortNumber
                    value={account.get('referrals_count')}
                    renderer={ReferralsCounter}
                  />
                </NavLink>
              </div>
            </div>
          ...
          {referralsEnabled && me === account.get('id') && <ReferralCodeTout />}
        </div>
